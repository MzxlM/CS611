...
//            System.out.println(Constant.ANSI_RED+ "Please input a number no less than 1 and no bigger than "+upperLimit+"."+Constant.ANSI_RESET);
    ...
    public String heroIconChecker() {
        ...
        try{
            ...
            checkQuit(input);
            if (input.length()>2){
                throw new Exception(addColor("Input should be length 2!", Constant.ANSI_RED));
...
//                throw new Exception(Constant.ANSI_RED+ "Your movement should only be WASD/wasd and I/i!"+Constant.ANSI_RESET);
    ...
    public String movementChecker(Hero hero) {
        System.out.println("Hero "+hero.getName()+" please enter your movement(WASD)");
        String input = "";
        try{
            input = sc.next();
            checkQuit(input);
...
//            System.out.println(Constant.ANSI_RED+ "Please input a number no less than 1 and no bigger than "+upperLimit+"."+Constant.ANSI_RESET);
    ...
    public boolean requirementCheck(Integer requirement, Integer heroStatus, String s) {
        //level check
